...
# Project Heading:      Statistical Analyses for Doctoral (PhD) Thesis
# Subheading:           Long Point Breeding Bird Data Analysis

# Required Libraries:   dplyr, ggplot2, scales, tidyverse

# Required input files: 
#  1. Bird territory data e.g., "base_bird_data.csv"

# Steps:
#  1. RUN UNIVARIATE MODELS FOR DATA EXPLORATION

# Output files: 
#  1. EXAMPLE: Table of univariate results across all scales for each variable. 
...
# Load packages required for work through list function (A), or individually (B)
# Option A
  list.of.packages <- c("lattice", "gridBase")


# Check to see packages are missing
  new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]


# Install the missing packages and load
...
# Import data and identify summary information
...
# Identify summary information from the data
  summary(base.bird.data)


# Identify and review the column names for variables within the data
  names(base.bird.data)

  
# -----------------------------------------------------------------------------#
  
# Visualize data with ggplot function
  
# Plot breeding bird territory data over time by site on line graphs
  summary.plot <- ggplot(data = base.bird.data,
                         aes(x = sample_year,
                             y = breeding_territories, color = site_code)) +
    geom_point() +  # Add a line plot
    facet_wrap(~ site_code, scales = "free_y") +  # Facet by site
    labs(x = "Year", y = "Abundance", color = "Site") +  # Label axes and legend
...
# Save plot and export to file type depending on requirement (i.e., supervisor, journal)
  ggsave()
...
# Filter breeding bird data by species interested in (or guild for complexity), example here is Song Sparrow (SOSP)
  SOSP.data <- base.bird.data %>%
    filter(species_code %in% c("SOSP"))  # Choose SOSP (Song Sparrow) as specific species
  
  
# Plot breeding bird territory data over time by site on line graphs
  SOSP.summary.plot <- ggplot(data = SOSP.data, aes(x = sample_year,
                                   y = breeding_territories, color = site_code)) +
    geom_point(aes(group = interaction(site_code, species_code))) +  # Line trends per site-species
    geom_line() +
    facet_wrap(~ site_code) +  # Create panels for each site, you can add "scales = "free_y" code to remove standard y-axis limits within plots
    labs(
      x = "Year",
      y = "Abundance",
      color = "Site",
...
# Filter breeding bird data by site interested in for viewing species, example here is RARO
  RARO.data <- base.bird.data %>%
    filter(site_code %in% c("RARO"))  # Choose RARO as specific site
  
  
# Plot breeding bird territory data over time by site on line graphs
  RARO.summary.plot <- ggplot(data = RARO.data, aes(x = sample_year,
                                                       y = breeding_territories, color = species_code)) +
    geom_point() +  
    geom_line() +
    facet_wrap(~ species_code, scales = "free_y") +  # Create panels for each site, you can add "scales = "free_y" code to remove standard y-axis limits within plots
    labs(
      x = "Year",
      y = "Abundance",
      color = "Species",
      title = "Species Trends Over Time at RARO"
    ) +
    theme_minimal()
...
# Provide site data as function for split
  site.data <- split(base.bird.data, base.bird.data$site_code)
