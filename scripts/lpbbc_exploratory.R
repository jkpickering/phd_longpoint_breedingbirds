...
#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#
# 3.0 Plot territory data over time (i.e., year) in a scatterplot with 
# 'facet_wrap'

# Visualize linear trend of total male breeding bird territories (i.e., 
# abundance) over time (years: 1991-2021) for all sites (n = 15)

# Summarize and create scatter plot with linear trendline
territories_over_year_by_site_linear_plot <- bird_territories %>% # set plot
  group_by(sitecode, # group by site
           year) %>% # subgroup by year
  summarise(total_territories = sum(territories, # sum site x year abundance
                                    na.rm = TRUE)) %>% # remove n.a. values
  ggplot(aes(x = year, # set x-axis data
             y = total_territories, # set y-axis data
             group = sitecode)) +  # group plot by site
  geom_smooth(method = "lm", size = 1, # set point size
  ...
  facet_wrap(~ sitecode, # separate plots by site
             ...
             scales = "free_y") +  # set y-axis values by plot
  labs(x = "Year", # set x-axis label
       y = "Number of male breeding bird territories") + # set y-axis label
  scale_y_continuous(limits = c(NA, NA), # set x-axis scale limits
                     expand = c(0, 0)) +  # set x-axis scale expansion points
  scale_x_continuous(breaks = seq(min(bird_territories$year), # set min x-axis
                                  max(bird_territories$year), # set max x-axis
                                  by = 5), # set gap x-axis breaks
                     limits = c(1990, 2022), # set x-axis scale limits
                     expand = c(0, 0)) # set x-axis scale expansion points

# Visualize the total territories (i.e., abundance) for each site over time
territories_over_year_by_site_linear_plot # visualize plot

#-----------------------------------------------------------------------------#
# 3.1 Save plot for record (OPTIONAL)
...
# "../results/plots/territories_over_year_by_site_plot.png"), # specify file location
# plot = territories_over_year_by_site_plot, # specify plot to save
# width = 8.18, # specify plot width
# height = 6.78) # specify plot height

#-----------------------------------------------------------------------------#
# 3.2 (OPTIONAL) Visualize and explore plots with 'ggplotly' function
# Remove written line function (i.e., '#') to run optional code
...
#-----------------------------------------------------------------------------#
# 3.3 Save plot for record (OPTIONAL)
# Remove written line function (i.e., '#') to run optional code
...
#-----------------------------------------------------------------------------#
# 4.0 Plot territory data over time (i.e., year) in a scatterplot with 
# 'facet_wrap'

# Visualize smoothed trend of total male breeding bird territories (i.e., 
# abundance) over time (years: 1991-2021) for all sites (n = 15)

# To filter specific sites only, include this code and then revise ncol format:
# filter(sitecode %in% c("ROWB", "ROMS", "ROIS", "RARO", "ROMF")) %>% 

# Summarize and create scatter plot with smoothed trendline
territories_over_year_by_site_smoothed_plot <- bird_territories %>% # set plot
  group_by(sitecode, # group by site
           year) %>% # subgroup by year
  summarise(total_territories = sum(territories, # sum site x year abundance
                                    na.rm = TRUE)) %>% # remove n.a. values
...
# Visualize the total territories (i.e., abundance) for each site over time
territories_over_year_by_site_smoothed_plot # visualize plot

#-----------------------------------------------------------------------------#
# 4.1 Save plot for record (OPTIONAL)
# Remove written line function (i.e., '#') to run optional code

# Save plot
# ggsave(filename = paste0(
  # "../results/plots/territories_over_year_by_site_smoothed_plot.png"),
  # plot = territories_over_year_by_site_smoothed_plot,
  # width = 8,
  # height = 6)

#-----------------------------------------------------------------------------#
# 4.2 (OPTIONAL) Visualize and explore plots with 'ggplotly' function
# Remove written line function (i.e., '#') to run optional code
...
# Convert ggplot to interactive plots
# territories_over_year_by_site_smoothed_plotly <- ggplotly(
# territories_over_year_by_site_smoothed_plot) %>%
  # layout(yaxis = list(tickfont = list(size = 10, # y-axis label text size
                                    # color = "black")), # y-axis label color
        # yaxis2 = list(tickfont = list(size = 10, # y-axis label text size
                                    # color = "black")), # y-axis label color
...
#-----------------------------------------------------------------------------#
# 4.3 Save plot for record (OPTIONAL)
# Remove written line function (i.e., '#') to run optional code

# Save plot
# saveWidget(territories_over_year_by_site_smoothed_plotly, 
           # file = "D:/r_projects/phd_longpoint_breedingbirds/results/plots/territories_over_year_by_site_smoothed_plotly.html",
           # selfcontained = TRUE)

#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#

# 5.0 Species richness: data visualizations
...
# 7.0 Breeding bird species plots: data visualizations

#-----------------------------------------------------------------------------#
# 7.1 Plot individual species trends over time, for individual sites
