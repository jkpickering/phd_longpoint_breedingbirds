...
# OPTIONAL: View data to review structure or contents
...
# 3.0 Model species abundance over time with generalized additive mixed models
# (GAMMs) for the study period, 1991-2021, at Long Point, Ontario, Canada

#-----------------------------------------------------------------------------#
# 3.1 Organize species data set in preparation for modeling with species
...
# for species abundance across time, 1991-2021 at Long Point, Ontario, Canada
#-----------------------------------------------------------------------------#
...
species_list <- unique(species_predictions$sp_common_name_uppercase_IBP) # list

# loop species to plot GAMM results
for (sp in species_list) { # specify loop for all species
  sp_pred <- subset(species_predictions, # specify species predictions
                    sp_common_name_uppercase_IBP == sp) # specify unique species
  
  p <- ggplot(sp_pred, # specify data set
              aes( # specify aesthetics
                x = year, # specify x-axis values
                y = fit)) + # specify y-axis values
    geom_line(
      color = "black", # specify color
      size = 1) + # specify size of response variable line
    geom_hline(
      yintercept = 0, # specify horizontal line location
      color = "black") +  # specify color
    geom_vline(
      xintercept = 1990, # specify vertical line location
      color = "black") + # specify color
    geom_ribbon(
      aes(ymin = fit - 1.96*se, # specify standard error value
          ymax = fit + 1.96*se), # specify standard error value
      alpha = 0.2, # specify size
      fill = "lightblue") + # specify fill color
    labs(
      title = paste(sp), # specify species for title
      x = "Year", # specify x-axis title
      y = "Predicted number of breeding bird territories, 1991-2021") + # title
    theme(
      legend.position = "none", # removes legend
      panel.background = element_rect(
        fill = "white", # specify color
        color = NA),  # specify panel color
      plot.background = element_rect(
        fill = "white", # specify color
        color = NA),   # specify plot color
      plot.title = element_text(
        size = 12, # specify text size
        face = "bold", # specify text format
        hjust = 0.95, # specify location of title
        color = "black", # specify color
        margin = margin(b = 15)), # specify spacing
      axis.title.x.bottom = element_text(
        size = 12, # specify text size
        face = "bold", # specify text form
        colour = "black", # specify color
        margin = margin(t = 15)), # spacing
      axis.title.y.left = element_text(
        size = 12, # specify text size
        face = "bold", # specify text format
        colour = "black", # specify color
        margin = margin(r = 15)), # spacing
      axis.text.x = element_text(
        size = 10, # specify text size
        color = "black"), # specify text color
      axis.text.y.left = element_text(
        size = 10, # specify text size
        color = "black")) + # specify color
    scale_y_continuous(
      limits = c(NA, NA), # specify y-axis limits
      expand = c(0, 0)) + # specify y-axis boundaries
    scale_x_continuous(
      limits = c(1990, 2022), # specify x-axis limits
      expand = c(0, 0)) # specify x-axis boundaries
  theme_pubr() # specify ggplot format, theme_pubr() is clean
  
  print(p)  # display plots in console
...
#-----------------------------------------------------------------------------#
# 3.3.1 Plot all species smooth trends with confidence ribbons and raw data
#-----------------------------------------------------------------------------#

# loop species to plot GAMM results
for (sp in species_list) {
  
  sp_pred <- subset(species_predictions, # Subset GAMM predictions
                    sp_common_name_uppercase_IBP == sp)
  
  sp_obs <- subset(bird_territories_with_traits, # Subset original data
                   sp_common_name_uppercase_IBP == sp)
  
  p <- ggplot(sp_pred, aes(
    x = year,
    y = fit)) + # Add original observations as points
    geom_point(data = sp_obs, 
               aes(x = year,
                   y = territories),  # Adjust 'observed_value' to your column name
               color = "darkgray", 
               alpha = 0.6, 
               size = 2) + 
...
# Loop over species models
for (sp in names(species_models)) {
  message(paste("Checking model for species:", sp))
  
  # Extract GAM model
  gam_model <- species_models[[sp]]$gam
  
  # Get k-check output
  k_check <- mgcv::k.check(gam_model)
  
  # Get model summary
  gam_summary <- summary(gam_model)
  
  # Optionally: extract global model metrics
  ...
  # Combine k-check into a tibble
  k_df <- as_tibble(k_check, rownames = "smoother_term") %>%
    mutate(species = sp) %>%
    relocate(species)
  
  # Optionally merge with model metrics (add same metrics to each row)
  k_df <- left_join(k_df, model_metrics, by = "species")
  
  # Save to list
  model_check_results[[sp]] <- k_df
}

# Combine all species results into one dataframe
model_check_table <- bind_rows(model_check_results)

# Preview the final table
print(model_check_table, n = 45)


# Suggestions for how to interpret results of model checks:
# A good fit will result in a k-index ≈ 1 and p-value > 0.05
...
# 4.0 Calculate and plot trend results from GAMMs for individual species for
# the study period, 1991-2021, at Long Point, Ontario, Canada

#-----------------------------------------------------------------------------#
# 4.1 Calculate the overall and annual percent change for each species
#-----------------------------------------------------------------------------#

# Calculate the annual rate of change (%) for the study period for each species
trend_results <- species_predictions %>% # specify predictions data set
  group_by(sp_common_name_uppercase_IBP) %>% # specify grouped requirement
  arrange(year, .by_group = TRUE) %>% # specify arrangement of group
  summarise( # calculate rates of change (overall and annual)
    overall_percent_change = (last(fit) / first(fit) - 1) * 100, # overall rate
    annual_percent_rate = { # annual rate of change calculation
      lm_fit <- lm(log(fit) ~ year) # rate of change (annual) 1/3
      rate <- coef(lm_fit)[2] # rate of change (annual) 2/3
      (exp(rate) - 1) * 100}) # rate of change (annual) 3/3

# Organize structure of results for increasing, decreasing, and stable trends
trend_results <- trend_results %>% # specify data set
  mutate(trend_category = case_when( # specify organization
    annual_percent_rate > 0.25 ~ "Increasing", # specify limits 
    annual_percent_rate < -0.25 ~ "Decreasing", # specify limits 
      TRUE ~ "Stable")) # specify limits

# Reorder species by annual trend
trend_results <- trend_results %>% # specify data set
  mutate(sp_common_name_uppercase_IBP = fct_reorder( # specify order
    sp_common_name_uppercase_IBP, # specify headers
    annual_percent_rate)) # specify data order

#-----------------------------------------------------------------------------#
# 4.2 Plot the annual percent change for each species
#-----------------------------------------------------------------------------#

#-----------------------------------------------------------------------------#
# 4.2.1 Plot a bar graph; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada

# Plot a bar graph for the rates of change for each species
annualchange.portrait <- ggplot(trend_results, # specify data set of trends
                            aes(x = sp_common_name_uppercase_IBP, # x-axis
                                y = annual_percent_rate, # specify y-axis
                                fill = trend_category)) + # specify fill values
  geom_col(show.legend = FALSE) + # remove legend
  coord_flip() +  # flip plot axes
  scale_fill_manual(values = c("Increasing" = "forestgreen", # specify color
                               "Stable" = "gray70", # specify color
                               "Decreasing" = "firebrick")) + # specify color
  labs(x = "Species", # specify x-axis label
    y = "Annual rate of change (%)") + # specify y-axis label
  theme_pubr() # specify ggplot format, theme_pubr() is clean

# Visualize bar chart
print(annualchange.portrait) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.2.1 Plot a bar graph; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
# Species names are color-coded for their identified nest guild

# Ensure only one row per species name
guild_lookup <- bird_territories_with_traits %>% # specify data set
  select(sp_common_name_uppercase_IBP, nest_guild_BOTW) %>% # specify category
  distinct(sp_common_name_uppercase_IBP, # specify unique species names
           .keep_all = TRUE) # specify keep unique species

# Then, left join only that column into trend_results
trend_results <- trend_results %>% # specify data set
  left_join(guild_lookup, # organize species guild
            by = "sp_common_name_uppercase_IBP") # sort by species

# Reorder species by annual trend
trend_results <- trend_results %>% # specify data set
  mutate(sp_common_name_uppercase_IBP = fct_reorder( # organize by species
    sp_common_name_uppercase_IBP, # specify species
    annual_percent_rate)) # organize by trend results

# OPTIONAL: Reorder and sub-order for annual trend for each guild
#trend_results <- trend_results %>% # specify data set
  #arrange(nest_guild_BOTW, # specify data set
          #annual_percent_rate) %>% # organize by species
  #mutate(sp_common_name_uppercase_IBP = factor( # organize by species
    #sp_common_name_uppercase_IBP, # specify species
    #levels = unique(sp_common_name_uppercase_IBP))) # organize by group

# Define custom guild colors - specify or revise to visualize unique trends
guild_colors <- c("cavity" = "black", # specify text color for guild
                  "ground" = "blue", # specify text color for guild
                  "tree" = "black", # specify text color for guild
                  "shrub"  = "blue") # specify text color for guild

# Match label color for each species
label_colors <- setNames(guild_colors[trend_results$nest_guild_BOTW], # list
                         trend_results$sp_common_name_uppercase_IBP) # trends

# Plot
annualchange.portrait.nest <- ggplot(trend_results, # specify data set
                                aes(x = sp_common_name_uppercase_IBP, # x value
                                    y = annual_percent_rate, # y-axis value
                                    fill = trend_category)) + # specify fill
  geom_col(show.legend = FALSE) + # remove legend
  coord_flip() + # flip plot axes
  scale_fill_manual(values = c("Increasing" = "forestgreen", # specify color
                               "Stable" = "gray70", # specify color
  ...
  labs(x = "Species", # specify x-axis label
       y = "Annual rate of change (%)") + # specify y-axis label
  theme_pubr() + # specify ggplot format, theme_pubr() is clean
  theme(axis.text.y = element_text(color = label_colors[levels( # specify color
      factor(trend_results$sp_common_name_uppercase_IBP))])) # specify text

# Visualize bar chart
print(annualchange.portrait.nest) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.2.2 Plot a scatterplot; portrait (vertical) for species' % annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
# Species names are bolded for their identified nesting guild

# Ensure only one row per species name
biome_lookup <- bird_territories_with_traits %>% # specify data set
  select(sp_common_name_uppercase_IBP, # specify data set
         Breeding.Biome) %>% # organize by biome
  distinct(sp_common_name_uppercase_IBP, # specify unique species
           .keep_all = TRUE) # specify to keep unique species

# Then, left join only that column into trend_results
trend_results <- trend_results %>% # specify data set
  left_join(biome_lookup, # join data sets
            by = "sp_common_name_uppercase_IBP") # join by species name

# Reorder species by annual trend
trend_results <- trend_results %>% # specify data set
  mutate(sp_common_name_uppercase_IBP = fct_reorder( # organize by species
    sp_common_name_uppercase_IBP, # specify unique species
    annual_percent_rate)) # organize by trend results

trend_results <- trend_results %>% # specify data set
  mutate(species_label_nguild = case_when( # specify labels
    nest_guild_BOTW == "ground" ~ paste0("<b>", # specify bold
                                                    sp_common_name_uppercase_IBP,
                                                    "</b>"),  # bold species name
    nest_guild_BOTW == "shrub" ~ paste0("<b>", # specify bold
                                         sp_common_name_uppercase_IBP,
                                         "</b>"),  # bold species name
    nest_guild_BOTW == "cavity" ~ sp_common_name_uppercase_IBP, # biome
    nest_guild_BOTW == "tree" ~ sp_common_name_uppercase_IBP, # biome
    TRUE ~ sp_common_name_uppercase_IBP)) # fail safe for biomes

trend_results$species_label_nguild <- factor( # specify data set
  trend_results$species_label_nguild, # specify factor
  levels = trend_results$species_label_nguild[order( # specify levels
    trend_results$annual_percent_rate)]) # specify label order

annualchange.portrait.biome.v2 <- ggplot(trend_results, # specify data set
                                         aes(x = species_label_nguild, # x-axis
                                             y = annual_percent_rate, # y-axis
                                             fill = trend_category)) + # specify fill
  geom_col(show.legend = FALSE) + # remove legend
  coord_flip() + # flip plot axes
  scale_fill_manual(values = c("Increasing" = "forestgreen", # specify color
  ...
  labs(x = "Species", # specify x-axis label
       y = "Annual rate of change (%)") + # specify y-axis label
  theme_pubr() + # specify ggplot format, theme_pubr() is clean
  geom_hline(yintercept = 0, # specify y-intercept location 
             linetype = "solid", # specify line type
             color = "gray50") + # specify line color
  coord_flip() + # flip plot axes
  labs(x = "Species", # specify x-axis text label
       y = "Annual rate of change (%)") + # specify y-axis text label
  theme(axis.text.y = ggtext::element_markdown(size = 10)) # specify text label

# Visualize bar chart
print(annualchange.portrait.biome.v2) # display plot in console

#-----------------------------------------------------------------------------#
#-----------------------------------------------------------------------------#
# 4.2.2.3 Plot a scatterplot; portrait (vertical) for species' % annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
# Species names are bolded for their identified species in 'decline'

# Ensure only one row per species name
status_lookup <- bird_territories_with_traits %>% # specify data set
  select(sp_common_name_uppercase_IBP, # specify data set
         conservation_status_BOTW) %>% # organize by biome
  distinct(sp_common_name_uppercase_IBP, # specify unique species
           .keep_all = TRUE) # specify to keep unique species

# Then, left join only that column into trend_results
trend_results <- trend_results %>% # specify data set
  left_join(status_lookup, # join data sets
            by = "sp_common_name_uppercase_IBP") # join by species name
...
trend_results <- trend_results %>% # specify data set
  mutate(species_label_status = case_when( # specify labels
    conservation_status_BOTW == "declining" ~ sp_common_name_uppercase_IBP, # biome
    conservation_status_BOTW == "common bird in steep decline" ~ paste0("<b>", # specify bold
                                                                        sp_common_name_uppercase_IBP,
                                                                        "</b>"),  # bold species name
    conservation_status_BOTW == "low concern" ~ sp_common_name_uppercase_IBP, # biome
    TRUE ~ sp_common_name_uppercase_IBP)) # fail safe for biomes

trend_results$species_label_status <- factor( # specify data set
  trend_results$species_label_status, # specify factor
  levels = trend_results$species_label_status[order( # specify levels
    trend_results$annual_percent_rate)]) # specify label order

annualchange.portrait.biome.v2 <- ggplot(trend_results, # specify data set
                                         aes(x = species_label_status, # x-axis
                                             y = annual_percent_rate, # y-axis
                                             fill = trend_category)) + # specify fill
  geom_col(show.legend = FALSE) + # remove legend
  coord_flip() + # flip plot axes
...
#-----------------------------------------------------------------------------#
# 4.2.3 Plot a bar graph; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
# Species names are color-coded for their identified breeding biome

# Join species information data "bird_territories_with_traits" with 
# territory trend data "trend_results" for analyses of guild
...
# OPTIONAL: Reorder and sub-order for annual trend for each guild
#trend_results <- trend_results %>% # specify data set
#arrange(nest_guild_BOTW, # organize by species' guild
        #annual_percent_rate) %>% # organize by trend results
#mutate(sp_common_name_uppercase_IBP = factor( # organize by species
        #sp_common_name_uppercase_IBP, # organize by species
        #levels = unique(sp_common_name_uppercase_IBP))) # specify by species

# Define custom guild colors - specify or revise to visualize unique trends
biome_colors <- c("Habitat Generalist" = "blue", # specify text color
                  "Eastern Forest" = "black", # specify text color
                  "Grassland" = "black", # specify text color
                  "Forest Generalist" = "blue", # specify text color
                  "Introduced" = "black", # specify text color
                  "Wetland" = "black", # specify text color
                  "Boreal Forest" = "black") # specify text color

# Match label color for each species
label_colors <- setNames(biome_colors[trend_results$Breeding.Biome], # biomes
                         trend_results$sp_common_name_uppercase_IBP) # results

# Plot
annualchange.portrait.biome <- ggplot(trend_results, # specify data set
                                aes(x = sp_common_name_uppercase_IBP, # x-axis
                                    y = annual_percent_rate, # y-axis values
                                    fill = trend_category)) + # specify fill
  geom_point(size = 3, # specify point size
             shape = 21, # specify point shape
             color = "white", # specify point color
             stroke = 1) + # specify point outline color
  geom_hline(yintercept = 0, # specify line intercept 
             linetype = "dashed", # specify line type
             color = "gray50") + # specify line color
  coord_flip() + # flip plot axes
  scale_fill_manual(values = c("Increasing" = "forestgreen", # specify color
                               "Stable" = "gray70", # specify color
  ...
  labs(x = "Species", # specify x-axis label
       y = "Annual rate of change (%)") + # specify y-axis label
  theme_pubr() + # specify ggplot format, theme_pubr() is clean
  theme(axis.text.y = element_text( # specify text color
    color = label_colors[levels( # specify color selection
      factor(trend_results$sp_common_name_uppercase_IBP))])) # specify name

# Visualize bar chart 
print(annualchange.portrait.biome) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.4 Plot a scatterplot; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
# Species names are bolded for their identified breeding biome

trend_results <- trend_results %>% # specify data set
  mutate(species_label_biome = case_when( # specify labels
    Breeding.Biome == "Habitat Generalist" ~ paste0("<b>", # specify bold
                                                  sp_common_name_uppercase_IBP,
                                                  "</b>"),  # bold species name
    Breeding.Biome == "Eastern Forest" ~ sp_common_name_uppercase_IBP, # biome
    Breeding.Biome == "Grassland" ~ sp_common_name_uppercase_IBP, # biome
    Breeding.Biome == "Forest Generalist" ~ paste0("<b>", # specify bold
                                                 sp_common_name_uppercase_IBP,
                                                 "</b>"),  # bold species name
    Breeding.Biome == "Introduced" ~ sp_common_name_uppercase_IBP, # biome
    Breeding.Biome == "Wetland" ~ sp_common_name_uppercase_IBP, # biome
    Breeding.Biome == "Boreal Forest" ~ sp_common_name_uppercase_IBP, # biome
    TRUE ~ sp_common_name_uppercase_IBP)) # fail safe for biomes

trend_results$species_label_biome <- factor( # specify data set
  trend_results$species_label_biome, # specify factor
  levels = trend_results$species_label_biome[order( # specify levels
    trend_results$annual_percent_rate)]) # specify label order

annualchange.portrait.biome.v2 <- ggplot(trend_results, # specify data set
                                      aes(x = species_label_biome, # x-axis
                                          y = annual_percent_rate, # y-axis
                                          fill = trend_category)) + # fill
  geom_point(size = 3, # specify point size
             shape = 21, # specify point shape
             color = "white", # specify point color
             stroke = 1) + # specify point outline color
...
#-----------------------------------------------------------------------------#
# 4.2.5 Plot a bar graph; landscape (horizontal) for species' percent annual 
# rate of change for the study period, 1991-2021 at Long Point, Ontario, Canada

# Plot a bar graph for the rates of change for each species
annualchange.landscape <- ggplot(trend_results, # plot rate of change trends
                            aes(x = sp_common_name_uppercase_IBP, # x-axis value
                                y = annual_percent_rate, # specify y-axis
                                fill = trend_category)) + # specify fill values
  ...
  theme(legend.position = "none", # Removes the legend
        panel.background = element_rect(fill = "white", # specify color
                                        color = NA),  # specify color
        plot.background = element_rect(fill = "white", # specify color
                                       color = NA), # specify color
        plot.title = element_text(size = 12, # specify size
                                  face = "bold", # specify text form
                                  hjust = 0.95, # specify label location
                                  color = "black", # specify color
                                  margin = margin(b = 15)), # specify spacing
        ...
        axis.text.x = element_text(size = 10, # specify text size
                                   angle = 90, # specify text angle
                                   hjust = 1, # specify location
                                   vjust = 0.5, # specify location
                                   color = "black"), # specify color
        axis.text.y.left = element_text(size = 10, # specify text size
                                        color = "black")) + # specify color
  scale_y_continuous(limits = c(-7, 12), # specify y-axis limits 
                     expand = c(0, 0))  # specify y-axis boundaries

# Visualize bar chart
print(annualchange.landscape) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.6 Plot a scatterplot; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada

# Plot a scatterplot for the rates of change for each species
rateofchange_plotv2 <- ggplot(trend_results, # specify data set
                              aes(x = annual_percent_rate, # specify x-axis
                                  y = fct_reorder(sp_common_name_uppercase_IBP,
                                                  annual_percent_rate), # y-axis
                                  color = trend_category)) + # specify fill
  geom_point(size = 3) + # specify point size
  geom_vline(xintercept = 0, # specify location
             linetype = "dashed", # specify line type
             color = "gray50") + # specify color
  scale_color_manual(values = c("Increasing" = "forestgreen", # specify color
                                "Stable" = "gray50", # specify color
                                "Decreasing" = "firebrick")) + # specify color
  labs(x = "Annual rate of change (%)", # specify x-axis label
    y = "Species") + # specify y-axis label
  theme_pubr() # specify ggplot format, theme_pubr() is clean

# Visualize plot
print(rateofchange_plotv2) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.7 Plot a bar graph; portrait (vertical) for species' percent overall rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada
...
overallchange_plot <- ggplot(trend_results, # plot rate of change trends
                            aes(x = sp_common_name_uppercase_IBP, # specify x-axis values
...
# Visualize plot
print(overallchange_plot) # display plot in console

#-----------------------------------------------------------------------------#
# 4.2.8 Plot a scatterplot; portrait (vertical) for species' percent annual rate 
# of change for the study period, 1991-2021 at Long Point, Ontario, Canada

# Faceted plot for both metrics
trend_results_long <- trend_results %>%
  select(sp_common_name_uppercase_IBP, annual_percent_rate, overall_percent_change, trend_category) %>%
  pivot_longer(cols = c("annual_percent_rate", "overall_percent_change"),
               names_to = "metric", values_to = "percent_change")

facet_plot <- ggplot(trend_results_long,
...
# Visualize plot
print(facet_plot) # display plot in console

#-----------------------------------------------------------------------------#
# 4.3 Calculate the annual change in abundance for each species
#-----------------------------------------------------------------------------#

# Total change = final year prediction - first year prediction
species_abundance_change <- species_predictions %>%
  group_by(sp_common_name_uppercase_IBP) %>%
  arrange(year) %>%
  summarise(
...
# Reorder species by annual trend
species_abundance_change <- species_abundance_change %>%
  mutate(sp_common_name_uppercase_IBP = fct_reorder(sp_common_name_uppercase_IBP, total_change))

#-----------------------------------------------------------------------------#
# 4.4 Plot the overall change in abundance for each species
#-----------------------------------------------------------------------------#

#-----------------------------------------------------------------------------#
# 4.4.1 Plot a bar graph; portrait (vertical) for species' overall change 
# for the study period, 1991-2021 at Long Point, Ontario, Canada
...
# 5.0 Breeding bird nesting guild plots: data visualizations
...
# 5.0 Breeding bird species plots: data visualizations - V Species Code

#-----------------------------------------------------------------------------#
# 5.1 Plot the overall percent change for each guild from a calculation of a 
...
species_data <- data.frame(
  Species = c("Species A", "Species B", "Species C", "Species D", "Species E", "Species F"),
  Biome = c("Forest", "Forest", "Grassland", "Grassland", "Wetland", "Wetland"),
  Guild = c("Canopy", "Ground", "Shrub", "Ground", "Canopy", "Ground"),
  Management_Group = c("Focal", "Generalist", "Focal", "Declining", "Invasive", "Focal"),
  stringsAsFactors = FALSE
)

ggplot(species_data, aes(x = Guild, y = Species, fill = Management_Group)) +
  geom_tile(color = "white", linewidth = 0.5) +
  facet_wrap(~ Biome, scales = "free_y") +
  scale_fill_brewer(palette = "Set2") +
  theme_minimal() +
  labs(title = "Species Groupings by Biome, Guild, and Management Group",
       x = "Nesting Guild",
       y = "Species") +
...
datatable(species_data, 
          ...
          caption = 'Species Grouped by Biome, Nesting Guild, and Management Group')

######################


# Entire abundance GAMM
...
# 2. EDF, significance of smooth terms
...
#########################
# OPTION 2 - summarize abundance measures
# does not really work due to sampling effort change (i.e., 1991, 2016 and 2021)
