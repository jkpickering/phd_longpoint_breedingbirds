...
# Subheading:     Data Exploration - Bird Abundance (territory) Data
...
# Plot the total number of sites sampled for each year over time using 'ggplot' function
sites.per.year.plot <- ggplot(sites.per.year, aes(x = year, y = sites_sampled)) +
  ...
  theme(legend.position = "none", # Removes the legend
        ...
        axis.text.x = element_text(size = 8, color = "black", angle = 45, hjust = 0.5, vjust = 0.5), # Formats x-axis text
  ...
  scale_x_continuous(breaks = seq(min(sites.per.year$year), max(sites.per.year$year), by = 1), expand = c(0,0.5)) + # Show all years on x-axis
  scale_y_continuous(breaks = seq(min(0), max(sites.per.year$sites_sampled), by = 1), expand = c(0,0))

# Visualize the total number of sites sampled for each year over time
