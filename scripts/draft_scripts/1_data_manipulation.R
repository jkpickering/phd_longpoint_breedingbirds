...
territory.plot <- ggplot(data = bird.territorydata,
                       aes(x = year,
...
SOSP.summary.plot <- ggplot(data = SOSP.data, aes(x = year,
                                                  y = territories, color = sitecode)) +
  geom_point(aes(group = interaction(sitecode, speciescode))) +  # Line trends per site-species
  geom_line() +
  facet_wrap(~ sitecode) +  # Create panels for each site, you can add "scales = "free_y" code to remove standard y-axis limits within plots
  labs(
    x = "Year",
    y = "Abundance",
    color = "Site",
...
# SUMMARY PLOTS (51) FOR A SINGLE SITE [RARO] ACROSS ALL YEARS, GROUPED ON ONE CHART (y = ABUNDANCE, x = YEAR)
# Visualize individual species data over time, grouped by site

# Filter breeding bird data by site interested in for viewing species, example here is RARO
RARO.data <- bird.territorydata %>%
  filter(sitecode %in% c("RARO"))  # Choose RARO as specific site


# Plot breeding bird territory data over time by site on line graphs
RARO.summary.plot <- ggplot(data = RARO.data, aes(x = year,
                                                  y = territories, color = speciescode)) +
  geom_point() +  
  geom_line() +
  facet_wrap(~ speciescode, scales = "free_y") +  # Create panels for each site, you can add "scales = "free_y" code to remove standard y-axis limits within plots
  labs(
    x = "Year",
    y = "Abundance",
    color = "Species",
...
RARO.species.plot <- lapply(names(RARO.species.data), function(species) {
  ggplot(RARO.species.data[[species]], aes(x = year, 
                                           y = territories, color = speciescode)) +
    geom_line(aes(group = speciescode)) +
