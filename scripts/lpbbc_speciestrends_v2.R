...
species_years <- aggregate(year ~ sp_common_name_uppercase_IBP, 
                           bird_territories_with_traits, 
                           function(x) length(unique(x)))

# Organize list into kept species
species_to_keep <- species_years$sp_common_name_uppercase_IBP[species_years$year > 4]

# Exclude specific species, such as Yellow Warbler (*outlier).
#species_to_keep <- bird_territories_with_traits %>%
  #group_by(sp_common_name_uppercase_IBP) %>%
  #summarise(n_years = n_distinct(year)) %>%
  #filter(n_years > 4, sp_common_name_uppercase_IBP != "Yellow Warbler") %>%
  #pull(sp_common_name_uppercase_IBP)
...
species_to_exclude <- c("American Goldfinch", # Summer breeder
...
species_to_keep <- bird_territories_with_traits %>%
  ...
  summarise(n_years = n_distinct(year),
    ...
    filter(n_years > 4,
           !sp_common_name_uppercase_IBP %in% species_to_exclude,
           !guild %in% c("lakes and ponds", "marshes", "shorelines")) %>%   # excludes these guilds
  pull(sp_common_name_uppercase_IBP)


data_filtered <- bird_territories_with_traits %>%
  filter(sp_common_name_uppercase_IBP %in% species_to_keep)
...
species_list <- unique(data_filtered$sp_common_name_uppercase_IBP) # create species list
...
for (sp in species_list) {
  sp_data <- subset(data_filtered, 
                    sp_common_name_uppercase_IBP == sp) # subset for individual species
...
trend_results <- trend_results %>%
  mutate(trend_category = case_when(
      annual_percent_rate > 0.25 ~ "Increasing",
      annual_percent_rate < -0.25 ~ "Decreasing",
      TRUE ~ "Stable"))

# Reorder species by annual trend
trend_results <- trend_results %>%
  mutate(sp_common_name_uppercase_IBP = fct_reorder(sp_common_name_uppercase_IBP, annual_percent_rate))

#-----------------------------------------------------------------------------#
# 3.5.1 Plot the annual percent change for each species

# Plot a bar graph for the rates of change for each species
annualchange.portrait <- ggplot(trend_results, # plot rate of change trends
...
# Visualize bar chart
print(annualchange.portrait)

#-----------------------------------------------------------------------------#
# 3.5.2 Plot the annual percent change for each species, color code nest guild
...
guild_lookup <- bird_territories_with_traits %>%
  select(sp_common_name_uppercase_IBP, nest_guild_BOTW) %>%
  distinct(sp_common_name_uppercase_IBP, .keep_all = TRUE)

# Then, left join only that column into trend_results
trend_results <- trend_results %>%
  left_join(guild_lookup, by = "sp_common_name_uppercase_IBP")

# Reorder species by annual trend
trend_results <- trend_results %>%
  mutate(sp_common_name_uppercase_IBP = fct_reorder(sp_common_name_uppercase_IBP,
                                                    annual_percent_rate))

# OPTIONAL: Reorder and sub-order for annual trend for each guild
#trend_results <- trend_results %>%
  #arrange(nest_guild_BOTW, annual_percent_rate) %>%
  #mutate(sp_common_name_uppercase_IBP = factor(sp_common_name_uppercase_IBP,
                                               #levels = unique(sp_common_name_uppercase_IBP)))
...
label_colors <- setNames(
  guild_colors[trend_results$nest_guild_BOTW],
  trend_results$sp_common_name_uppercase_IBP)

# Plot
annualchange.portrait.nest <- ggplot(trend_results,
                                aes(x = sp_common_name_uppercase_IBP,
  ...
  theme(
    axis.text.y = element_text(
      color = label_colors[levels(factor(trend_results$sp_common_name_uppercase_IBP))]  # apply color by species
    ))

# Visualize bar chart
print(annualchange.portrait.nest)

#-----------------------------------------------------------------------------#
# 3.5.3 Plot the annual percent change for each species, color code the biome

# Join species information data "bird_territories_with_traits" with 
# territory trend data "trend_results" for analyses of guild

# Ensure only one row per species name
biome_lookup <- bird_territories_with_traits %>%
  select(sp_common_name_uppercase_IBP, Breeding.Biome) %>%
  distinct(sp_common_name_uppercase_IBP, .keep_all = TRUE)

# Then, left join only that column into trend_results
trend_results <- trend_results %>%
  left_join(biome_lookup, by = "sp_common_name_uppercase_IBP")
...
# Define custom guild colors - specify or revise to visualize unique trends
biome_colors <- c(
  "Habitat Generalist" = "black",
  "Eastern Forest" = "blue",
  "Grassland" = "black",
  "Forest Generalist" = "black",
  "Introduced" = "black",
  "Wetland" = "black",
  "Boreal Forest" = "black")
...
# Match label color for each species
label_colors <- setNames(
  biome_colors[trend_results$Breeding.Biome],
  trend_results$sp_common_name_uppercase_IBP)

# Plot
annualchange.portrait.biome <- ggplot(trend_results,
                                aes(x = sp_common_name_uppercase_IBP,
                                    y = annual_percent_rate,
...
# Visualize bar chart
print(annualchange.portrait.biome)

#-----------------------------------------------------------------------------#
# 3.5.3.1 Plot the annual percent change for each species, bold the biome

# Join species information data "bird_territories_with_traits" with 
# territory trend data "trend_results" for analyses of guild

trend_results <- trend_results %>%
  mutate(species_label_biome = case_when(
    Breeding.Biome == "Habitat Generalist" ~ paste0("<b>", sp_common_name_uppercase_IBP, "</b>"),  # bold
    Breeding.Biome == "Eastern Forest" ~ sp_common_name_uppercase_IBP,
    Breeding.Biome == "Grassland" ~ sp_common_name_uppercase_IBP,
    Breeding.Biome == "Forest Generalist" ~ sp_common_name_uppercase_IBP,
    Breeding.Biome == "Introduced" ~ sp_common_name_uppercase_IBP,
    Breeding.Biome == "Wetland" ~ sp_common_name_uppercase_IBP,
    Breeding.Biome == "Boreal Forest" ~ sp_common_name_uppercase_IBP,
    TRUE ~ sp_common_name_uppercase_IBP  # fallback in case of NA or unmatched biome
  ))

trend_results$species_label_biome <- factor(
  trend_results$species_label_biome,
  levels = trend_results$species_label_biome[order(trend_results$annual_percent_rate)])

annualchange.portrait.biome.v2 <- ggplot(trend_results,
                                      aes(x = species_label_biome,
                                          y = annual_percent_rate,
                                          fill = trend_category)) +
  geom_point(size = 2,
             shape = 21,
  ...
  labs(x = "Species",
       y = "Annual rate of change (%)") +
  theme_pubr() +
  theme(axis.text.y = ggtext::element_markdown(size = 10)  # now supports <b> tags
  )

print(annualchange.portrait.biome.v2)

#-----------------------------------------------------------------------------#
# 3.5.4 Plot the overall change and annual percent change for each species (V2)
...
# Visualize bar chart
print(annualchange.landscape)

#-----------------------------------------------------------------------------#
# 3.5.4 Plot the overall change and annual percent change for each species (V3)
...
ggplot(trend_results_long, aes(x = percent_change, y = fct_reorder(sp_common_name_uppercase_IBP, percent_change), color = trend_category)) +
  geom_point(size = 3) +
...
# 4.0 Breeding bird species plots: data visualizations - V Species Code

#-----------------------------------------------------------------------------#
# 4.1 Organize species dataset in preparation for GAMM
...
#-----------------------------------------------------------------------------#
# 4.2 Model with loop through for species to fit GAMM and generate predictions
...
# 4.3 Plot all species smooth trends with confidence ribbons

# create unique species list
species_list <- unique(species_predictions$species_4code_IBP)
...
#-----------------------------------------------------------------------------#
# 5.0 Plot the overall percent change for each guild from a calculation of a 
